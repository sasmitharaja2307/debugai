...
function DiffRow({ file, type }) {
  const icons = {
    added:    { icon: Plus,   cls: 'text-green-600',  bg: 'bg-green-50 border-green-200'   },
    removed:  { icon: Minus,  cls: 'text-red-500',    bg: 'bg-red-50 border-red-200'       },
    modified: { icon: Edit3,  cls: 'text-amber-600',  bg: 'bg-amber-50 border-amber-200'   },
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-slate-800">Code State Tracker</h2>
      ...
      {lastGood && (
        <div className="card border-green-300">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-semibold text-green-600">Last Successful Run</span>
          </div>
          <p className="text-xs text-slate-500 font-mono">{lastGood.command}</p>
          <p className="text-xs text-slate-400 mt-0.5">{formatDate(lastGood.timestamp)}</p>
          <p className="text-xs text-slate-400 mt-1">{lastGood.file_count} files tracked</p>
        </div>
      )}
      ...
      <div className="card">
        ...
        {history.length === 0 ? (
          <p className="text-center text-slate-400 text-sm py-6">No snapshots yet. Run a command to start tracking.</p>
        ) : (
            ...
            {[...history].reverse().map((snap, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-mono text-slate-700 truncate">{snap.command}</p>
                  <p className="text-xs text-slate-400 mt-0.5">{formatDate(snap.timestamp)}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-xs text-slate-400">{snap.file_count} files</p>
